...
public class Cylinder {
	...
	public void DrawCylinder(float radius, float height, int slices) {
		...
		for (int i = 0; i < slices; i++) {
			...
			topVertices[i] = new Point4f(x, height / 2, z, 1.0f);
			bottomVertices[i] = new Point4f(x, -height / 2, z, 1.0f);
		}
		...
		for (int i = 0; i <= slices; i++) {
			int index = i % slices;
			Vector4f normal = bottomVertices[index].MinusPoint(new Point4f(0, -height / 2, 0, 1.0f)).Normal();
		...
		drawCircle(topVertices, new Point4f(0, height / 2, 0, 1.0f));
		drawCircle(bottomVertices, new Point4f(0, -height / 2, 0, 1.0f));
	}
